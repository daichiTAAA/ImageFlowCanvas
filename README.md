...
- **Kubernetes/Nomadネイティブ**: K3s/Nomad + 直接gRPC呼び出しによる堅牢で高性能な実行基盤
...
```bash
# 開発用にWebアプリがホットリロードされるようにして起動。
# アクセス: `http://localhost:3000`（外部は3000のみ公開。3001は公開しません）
# 仕組み: コンテナ内の nginx が 3000 を待受し、内部の Vite 開発サーバー `127.0.0.1:3001` へプロキシします（ブラウザは3001に直接アクセスしません）。
# HMR: Vite は HMR クライアントを 3000 に向けています（`hmr.clientPort=3000`）。ページのリロードなしで変更が反映されます。
