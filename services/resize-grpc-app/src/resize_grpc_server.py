...
class ResizeServiceImplementation(resize_pb2_grpc.ResizeServiceServicer):
    ...
    def ResizeImage(self, request, context):
        ...
        logger.info(
            f"Processing resize request for execution_id: {request.execution_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"Resize completed successfully - Total: {total_time:.2f}s (download: {download_time:.2f}s, processing: {processing_time:.2f}s, upload: {upload_time:.2f}s)"
            )
            return response
            ...
            total_time = time.time() - start_time
            logger.error(
                f"Error processing resize request (after {total_time:.2f}s): {str(e)}"
            )
...
def serve():
    ...
    logger.info(
        f"Starting optimized gRPC Resize Service on {listen_addr} with {max_workers} workers"
    )
    server.start()
