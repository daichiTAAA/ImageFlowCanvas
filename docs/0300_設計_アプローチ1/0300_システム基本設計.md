...
graph TB
    subgraph "外部層"
        KMPApp[Kotlin Multiplatformアプリ<br/>• Desktop/Mobile/Wearable<br/>• gRPC/WebSocket/REST/<br>WHEP/WebRTC<br/>• デバイス統合管理]
...
注: 現場（工場/ライン）の無線LAN設計・ローミング方針は `0307_インフラ設計.md` の「8.1.3. 現場ネットワーク設計（小型AP＋有線バックホール）」を参照。
...
- 詳細設計・運用手順は `0307_インフラ設計.md` 8.1.3章に集約。
