...
LLM連携画像処理パイプラインは以下のコンポーネントで構成される。

- **LLMフロントエンド**: 日本語指示をLLMへ送信しパイプライン定義を生成。
- **ツールレジストリ**: 既存gRPCサービスをメタデータ付きで管理。
- **パイプライン生成サービス**: LLM出力を解析しワークフロー定義を保存。
- **MLRunオーケストレータ**: 生成されたワークフローの実行と状態管理を担当。
- **推論サーバ**: モデル推論を提供しオーケストレータから呼び出される。
- **既存Pipeline Executor**: 実行結果を既存の高速gRPC経路へ統合。

処理フローは「自然言語入力 → LLM解析 → 定義保存 → ワークフロー実行 → 結果返却」である。
...
| 処理エンジン     | gRPC常駐サービス        | 既存Pipeline Executorによる超高速パイプライン実行 | 40-100ms処理、Backend経由でアクセス、Protocol Buffers |
...
| メタデータ管理   | ツールレジストリ        | gRPCサービスのメタデータ管理               | コンポーネント誤使用防止                              |
| パイプライン生成 | LLMサービス             | 自然言語をワークフロー定義へ変換           | ユーザー指示から自動生成                              |
| オーケストレーション | MLRun               | ワークフロー実行・状態管理                 | 生成パイプラインを統括                                |
| AI推論           | Triton Inference Server | モデル推論を提供しオーケストレータから呼び出される | GPU最適化、モデル管理、並列処理                       |
...
CREATE TABLE product_master (
    ...
    INDEX idx_product_code_machine (product_code, machine_number),
    INDEX idx_production_date (production_date),
    UNIQUE KEY uk_product_sequence (product_code, machine_number, production_date, monthly_sequence)
);
```
...
(product_code, machine_number, recorded_date, inspection_result);
