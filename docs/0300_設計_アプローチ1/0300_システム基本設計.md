...
graph TB
    subgraph "外部層"
        ...
        ComposeApp[Compose Multiplatformアプリ<br/>• Desktop/Mobile/Wearable<br/>• gRPC/WebSocket/REST<br/>• デバイス統合管理]
        WebClient[Webブラウザ<br/>• 管理・監視<br/>• WebSocket/REST<br/>• gRPC制約あり]
        Camera[カメラ・IoTデバイス<br/>• USB/API接続<br/>• Tauri/Compose経由]
    end
    ...
    Camera -->|USB/API<br/>映像取得| ComposeApp
    ...
    %% Compose Multiplatformアプリの通信
    ComposeApp -->|gRPC<br/>パイプライン実行<br/>高性能| Gateway
    ComposeApp -->|WebSocket<br/>進捗通知<br/>リアルタイム| Gateway
    ComposeApp -->|REST API<br/>マスタ取得<br/>シンプル| Gateway
    ComposeApp -->|WHEP/WebRTC<br/>リアルタイム映像<br/>超低遅延| Gateway
...
| 層                     | 技術                           | 役割                                 | 選定理由                                             |
| ---------------------- | ------------------------------ | ------------------------------------ | ---------------------------------------------------- |
| クロスプラットフォーム | Tauri + Rust + React           | デスクトップアプリ・モバイル         | ネイティブ性能、gRPC直接通信、Webview + Rust Core    |
| マルチプラットフォーム | Kotlin Multiplatform + Compose | デスクトップ・モバイル・ウェアラブル | 統一コードベース、ネイティブパフォーマンス、gRPC対応 |
...
| ----------- | -------------------------- | ---------------------------- | -------------------------------------------- |
| gRPC        | AIパイプライン実行         | Tauri/Compose ↔ Backend API  | 最高性能、型安全性、バイナリ転送             |
...
| WHEP/WebRTC | 超低遅延映像ストリーミング | Tauri/Compose ↔ Backend API  | 最低遅延、メディア特化、カメラ統合管理       |
| USB/API     | カメラ映像取得             | カメラ ↔ Tauri/Compose       | デバイス直接制御、高画質、安定性             |
...
- 通信: Tauri/Compose MultiplatformアプリからBackend APIへgRPC（高性能・型安全）
...
- 用途: 処理進捗のリアルタイム表示
- 通信: Backend APIから全クライアント（Tauri + Compose + Web）へWebSocket
...
- 通信: カメラ → Tauri/Compose（USB/API） → Backend API（WebRTC）
- 処理方式: Tauri/ComposeがカメラをネイティブAPIで制御し、映像をWebRTCでサーバーに転送
- 性能: <20ms 超低遅延
...
| 📹 カメラ統合管理        | Tauri/ComposeアプリがカメラをUSB/API制御し、WebRTCでサーバーに転送する統合アーキテクチャ | USB, API, WebRTC    |
...
| プラットフォーム | Tauri | Kotlin Multiplatform | 主要用途                   | 選定理由                                      |
| ---------------- | ----- | -------------------- | -------------------------- | --------------------------------------------- |
| Windows Desktop  | ✅     | ✅                    | 管理業務、パイプライン設計 | 既存Webアセット活用 / ネイティブ性能          |
| macOS Desktop    | ✅     | ✅                    | 管理業務、パイプライン設計 | 既存Webアセット活用 / ネイティブ性能          |
| Linux Desktop    | ✅     | ✅                    | 管理業務、パイプライン設計 | 既存Webアセット活用 / ネイティブ性能          |
| Android Mobile   | ✅     | ✅                    | 現場検査、品質管理         | Tauri 2.0対応 / Kotlin MPがモバイル最適       |
| iOS Mobile       | ✅     | ✅                    | 現場検査、品質管理         | Tauri 2.0対応 / Kotlin MPがモバイル最適       |
| Wear OS          | 🔶     | ✅                    | クイック確認、アラート     | Tauri技術的可能・要検証 / Kotlin MPが専用対応 |
| watchOS          | 🔶     | ✅                    | クイック確認、アラート     | Tauri技術的可能・要検証 / Kotlin MPが専用対応 |

---

# 6. Backend APIのハイブリッド実装
...
7. WebRTC映像処理: Tauri/Composeアプリからの映像ストリームを受信し、AI処理パイプラインに転送
