...
graph TB
    ...
    Backend -.->|直接gRPC呼び出し| AIService
    Backend -.->|直接gRPC呼び出し| FilterService
    
    %% リアルタイム処理フロー
    Camera -->|gRPC Stream<br/>リアルタイム映像| Gateway
    Gateway -.->|ストリーミング処理| ResizeService
    Gateway -.->|ストリーミング処理| AIService
...
| 処理エンジン   | gRPC常駐サービス | 超高速パイプライン実行         | 40-100ms処理、直接呼び出し、Protocol Buffers |
...
| 処理速度       | 40-100ms (直接gRPC実行)    | gRPC常駐サービス直接呼び出し |
| リアルタイム性 | <50ms (ストリーミング処理) | gRPCストリーミング           |
...
- **処理方式**: 直接gRPC呼び出し（メイン）、Kafkaフォールバック
- **性能**: 40-100ms処理時間
- **特徴**: ファイルベース、永続化、監査ログ
...
| **🚀 gRPC**             | Googleが開発した高性能なRPCフレームワーク。直接呼び出しで40-100ms処理を実現    | HTTP/2, Protobuf |
...
| **⚡ Direct gRPC Call** | Kafkaを経由せずgRPCサービスを直接呼び出す高速実行方式                          | gRPC, 同期処理   |
