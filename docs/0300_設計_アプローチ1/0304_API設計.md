...
### 3.3.1. エンドポイント一覧（/api/v1/uplink/*）

| 区分       | パス                                             | メソッド | 概要                                        |
| ---------- | ------------------------------------------------ | -------- | ------------------------------------------- |
| デバイス   | `/api/v1/uplink/devices/register`                | POST     | THINKLETデバイスの登録                      |
| セッション | `/api/v1/uplink/sessions`                        | POST     | インジェストセッション作成（WHEP発行）      |
| セッション | `/api/v1/uplink/sessions/{session_id}/telemetry` | POST     | テレメトリ/ハートビート送信                 |
| 制御       | `/api/v1/uplink/control`                         | POST     | 録画開始/停止・プライバシー停止・QoS切替    |
| 検査連携   | `/api/v1/uplink/inspections/start`               | POST     | 検査コンテキスト開始（製品情報紐づけ）      |
| セグメント | `/api/v1/uplink/segments/close`                  | POST     | 検査セグメント確定（メタ/オブジェクト紐付） |
...
エンドポイント: `POST /api/v1/uplink/sessions/{session_id}/telemetry`

目的: 接続状態・QoS・装着状態・視野角などを時系列で受信し、`uplink_stream_metrics` に蓄積。

リクエスト（例）:
```json
POST /api/v1/uplink/sessions/b28f.../telemetry
Content-Type: application/json
...
エンドポイント: `POST /api/v1/uplink/control`
