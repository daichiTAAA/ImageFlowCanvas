...
#### 3.1.2.1. 検査指示一覧取得API
```
GET /v1/inspection-instructions?page=1&per_page=20
...
#### 3.1.2.2. 検査指示詳細取得API
```
GET /v1/inspection-instructions/{id}
...
## 3.3. カメラ映像 API（統合映像管理基盤）

カメラデバイスの映像を統合管理するAPI群。

### 3.3.1. エンドポイント一覧（/v1/uplink/*）

| 区分       | パス                                         | メソッド | 概要                                        |
| ---------- | -------------------------------------------- | -------- | ------------------------------------------- |
| デバイス   | `/v1/uplink/devices/register`                | POST     | THINKLETデバイスの登録                      |
| セッション | `/v1/uplink/sessions`                        | POST     | インジェストセッション作成（WHEP発行）      |
| セッション | `/v1/uplink/sessions/{session_id}/telemetry` | POST     | テレメトリ/ハートビート送信                 |
| 制御       | `/v1/uplink/control`                         | POST     | 録画開始/停止・プライバシー停止・QoS切替    |
| 検査連携   | `/v1/uplink/inspections/start`               | POST     | 検査コンテキスト開始（製品情報紐づけ）      |
| セグメント | `/v1/uplink/segments/close`                  | POST     | 検査セグメント確定（メタ/オブジェクト紐付） |
...
エンドポイント: `POST /v1/uplink/sessions`
...
{
  "session_id": "b28f...",
  "whep_url": "https://ingest.example.local/whep/uplink/THINKLET-001",
  "auth_token": "eyJhbGci...",
...
エンドポイント: `POST /v1/uplink/sessions/{session_id}/telemetry`

目的: 接続状態・QoS・装着状態・視野角などを時系列で受信し、`uplink_stream_metrics` に蓄積。

リクエスト（例）:
```json
POST /v1/uplink/sessions/b28f.../telemetry
Content-Type: application/json
...
エンドポイント: `POST /v1/uplink/control`
...
| 区分   | パス                             | メソッド | 概要                            |
| ------ | -------------------------------- | -------- | ------------------------------- |
| 登録   | `/v1/privacy-zones/beacons`      | POST     | ビーコン登録                    |
| 一覧   | `/v1/privacy-zones/beacons`      | GET      | ビーコン一覧・検索              |
| 詳細   | `/v1/privacy-zones/beacons/{id}` | GET      | ビーコン詳細                    |
| 更新   | `/v1/privacy-zones/beacons/{id}` | PUT      | 識別子/交換予定日の更新等       |
| 健全性 | `/v1/privacy-zones/health`       | GET      | last_seen/vbattの健全性レポート |

セキュリティ: 管理者ロールのみ。全操作は監査記録。

## 3.4. VMS 映像管理 API（/v1/vms/*）
...
| 区分       | パス                                     | メソッド | 概要                              |
| ---------- | ---------------------------------------- | -------- | --------------------------------- |
| 検索       | `/v1/vms/search`                         | GET      | セグメント検索                    |
| 詳細       | `/v1/vms/segments/{segment_id}`          | GET      | セグメント詳細取得                |
| 再生       | `/v1/vms/segments/{segment_id}/playback` | GET      | 署名URL or Viewerトークン発行     |
| ライブ視聴 | `/v1/vms/live/{source}/{id}/whep`        | GET      | ライブ視聴用WHEP URL/トークン発行 |

source 例: `thinklet | cctv | terminal`。`id` はソースごとの識別子（デバイスID等）。

### 3.4.2. 検索API

エンドポイント: `GET /v1/vms/search`
  ...
  "items": [
    {
      ...
      "quality_status": "PASS",
      "object_key": "vms/uplink/THINKLET-001/2025/08/24/01/...mp4"
    }
  ],
  "total": 1
}
...
エンドポイント: `GET /v1/vms/segments/{segment_id}/playback`
...
```json
{
  "playback": {
    "type": "signed_url",
    "url": "https://minio.example.local/vms/uplink/...mp4?X-Amz-Expires=1800"
  }
}
```
  ...
  "output_files": [
    {
      "file_id": "output-123",
      "filename": "result.jpg",
      "download_url": "/v1/files/output-123/download"
    }
  ]
}
...
**QRコードスキャン履歴**
```http
GET /v1/products/qr-scan-history
GET /v1/products/qr-scan-history/{history_id}
```
...
**オートコンプリート**
```http
GET /v1/products/autocomplete/{field}?q={query}
```
...
**検査結果検索（製品情報ベース）**
```http
GET /v1/inspection/results/by-product/{product_id}
POST /v1/inspection/results/search
```
...
{
  "product_id": "prod-uuid-12345",
  "video_file_path": "/videos/uplink/VIDEO_001.mp4",
...
**映像検索（製品情報ベース）**
```http
GET /v1/videos/by-product/{product_id}
...
**製品トレーサビリティ記録**
```http
POST /v1/traceability/records
```

**製品トレーサビリティ検索**
```http
GET /v1/traceability/products/{product_id}
POST /v1/traceability/search
```
...
**検査指示マスタ**
```http
GET /v1/inspection/instructions
POST /v1/inspection/instructions
GET /v1/inspection/instructions/{instruction_id}
PUT /v1/inspection/instructions/{instruction_id}
DELETE /v1/inspection/instructions/{instruction_id}
```

**検査項目マスタ（製品タイプ別）**
```http
GET /v1/inspection/instructions/{instruction_id}/items
GET /v1/inspection/items/by-product-code/{product_code}
POST /v1/inspection/instructions/{instruction_id}/items
GET /v1/inspection/items/{item_id}
PUT /v1/inspection/items/{item_id}
DELETE /v1/inspection/items/{item_id}
```
...
```http
GET /v1/inspection/sessions/{session_id}
PUT /v1/inspection/sessions/{session_id}
POST /v1/inspection/sessions/{session_id}/complete
```

**AI検査実行（製品情報自動紐づけ）**
```http
POST /v1/inspection/sessions/{session_id}/ai-inspection
```
...
**人による検証API（製品情報自動継承）**
```http
POST /v1/inspection/sessions/{session_id}/human-verification
```
...
**オフライン同期**
```http
POST /v1/inspection/sync/upload
GET /v1/inspection/sync/download
```

**差分同期**
```http
GET /v1/inspection/sync/changes?since={timestamp}
POST /v1/inspection/sync/apply-changes
```
...
**基本統計情報**
```http
GET /v1/inspection/statistics?start_date={date}&end_date={date}
```
...
**リアルタイム品質指標**
```http
GET /v1/inspection/quality/realtime
WebSocket: /ws/quality-metrics
```

**検査傾向分析**
```http
GET /v1/inspection/quality/trends
GET /v1/inspection/quality/alerts
```
...
**検査項目とパイプラインの関連付け**
```http
GET /v1/inspection/items/{item_id}/pipeline
PUT /v1/inspection/items/{item_id}/pipeline
```

**パイプライン実行結果の検査結果への変換**
```http
POST /v1/inspection/pipeline-results/convert
```
...
**デバイス登録・認証**
```http
POST /v1/kmp/devices/register
POST /v1/kmp/auth/login
```
...
```http
POST /v1/ingest/sessions/{session_id}/stop
Authorization: Bearer <JWT>
...
```http
PUT /v1/upload/segments/{segment_id}/parts/{part_number}
Authorization: Bearer <JWT>
...
```http
POST /v1/upload/segments/{segment_id}/complete
Authorization: Bearer <JWT>
Content-Type: application/json
...
```http
GET /v1/devices/{device_id}/unsent-queue
Authorization: Bearer <JWT>
...
```http
GET /v1/devices/{device_id}/bandwidth-hint?network=wifi
Authorization: Bearer <JWT>
...
```json
{
  ...
  "instance": "/v1/inspection/sessions/session-123/ai-inspection",
  "validation_errors": [
    {
...
- STUN/TURN設定取得: `GET /v1/rtc/config`（ICEサーバ一覧と資格情報を返却）
...
フロー:
1) `POST /v1/recordings/uploads/presign` → `url`, `fields`, `part_urls[]` を返却
2) クライアントはマルチパートPUT（ETag収集）
3) `POST /v1/recordings/uploads/complete`（part番号とETag列挙）
  ...
  "videos": [
    {
      ...
      "thumbnail_url": "/v1/videos/video-001/thumbnail",
      "created_at": "2025-08-08T10:00:00Z"
    }
  ],
  "pagination": {
...
GET /v1/videos/{video_id}/stream.m3u8        # HLS
GET /v1/videos/{video_id}/stream.mpd         # DASH
GET /v1/videos/{video_id}/thumbnail          # サムネイル
GET /v1/videos/{video_id}/metadata           # メタデータ
```
...
**エンドポイント**:
```
GET    /v1/videos/{video_id}/bookmarks       # ブックマーク一覧
POST   /v1/videos/{video_id}/bookmarks       # ブックマーク作成
PUT    /v1/videos/{video_id}/bookmarks/{id}  # ブックマーク更新
DELETE /v1/videos/{video_id}/bookmarks/{id}  # ブックマーク削除
```
...

{
  ...
  "report_id": "report-001",
  "status": "COMPLETED",
  "download_url": "/v1/analytics/reports/report-001/download",
