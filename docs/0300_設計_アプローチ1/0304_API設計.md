...
### 1.2.2. 端末アプリ（ハンディーターミナル/デスクトップ）→ gRPC ストリーミング
...
```
- マッピング仕様（重要）:
  - `ProductMaster.product_code` と `InspectionTarget.product_code` を一致キーとして紐づける。
  - 一致しない場合、フォールバックとして `ProductMaster.work_order_id` と `InspectionTarget.product_code` の一致も試行する。
  - 対応する `InspectionTarget` が無い場合は `items=[]` を返す（404ではなく空配列）。
...
グループと検査ターゲット（InspectionTarget）の関係:
- `InspectionTarget.group_id` により、グループ単位で検査項目（InspectionItem）を定義できる。
- 端末アプリから製品に対する検査項目取得時は、以下の順でマッピングする。
  1) `product_code` が所属するグループを探索（基準は ProductMaster.`product_code`）→ `InspectionTarget.group_id` が一致するターゲットを優先
  2) フォールバック: `InspectionTarget.product_code` ≡ 製品`product_code`（＝ProductMaster.`product_code`）
  3) さらにフォールバック: `InspectionTarget.product_code` ≡ `product.work_order_id`
...
4) グループと検査対象の設定例（InspectionTarget）
```
{
  "name": "グループA向け検査対象",
  "group_id": "5f4c...",
