...
### 14.3.2. 変更 API（検査マスタ）
- 旧: `POST /v1/inspection/targets`（body に `product_code` or `group_id`）
- 新: `POST /v1/inspection/targets`（body に必須 `group_id`, `process_code`）
  - 例:
  ```json
  {
    "name": "外観検査 v1",
...
- 取得/一覧
  - `GET /v1/inspection/targets?group_id=...&process_code=...&page=...`
