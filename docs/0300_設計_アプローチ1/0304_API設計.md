...
- **🖥️ Kotlin Multiplatformアプリケーション**: ネイティブgRPC接続によるストリーミング処理（低レイテンシ、高性能）
- **🌐 Webブラウザクライアント**: WebSocket接続による映像ストリーミング（ブラウザ制約対応）

```mermaid
sequenceDiagram
    participant KmpApp as 🖥️ Kotlin Multiplatformアプリ<br/>(ネイティブ)
    ...
    Note over KmpApp, Kafka: 1. リアルタイム映像ストリーミング（クライアント別プロトコル）
    
    %% Kotlin Multiplatform gRPC接続
    KmpApp->>Gateway: gRPC Stream<br/>VideoFrame{<br/>  frame_data: bytes,<br/>  timestamp_ms: 1234567890,<br/>  metadata: {<br/>    source_id: "cam-001",<br/>    pipeline_id: "realtime-detect"<br/>  }<br/>}
    ...
    Note over KmpApp, Kafka: 2. リアルタイム推論（<50ms）
    ...
    Note over KmpApp, Kafka: 3. 結果ストリーミング（クライアント別配信）
    StreamService-->>Gateway: gRPC Stream返却
    
    %% Kotlin Multiplatform gRPC配信
    Gateway-->>KmpApp: gRPC Stream<br/>ProcessedFrame返却
    ...
    Note over KmpApp, Kafka: 4. 選択的アーカイブ（オプション）
    StreamService-->>Kafka: 映像アーカイブ<br/>Topic: video-stream-archive<br/>（重要フレームのみ）
    
    Note over KmpApp, Kafka: 5. 監視ダッシュボード更新
    StreamService-->>KmpApp: gRPC Status通知
...
| :------------------------------- | :------------- | :------------------------------------------------------------------------------------------------------------------------------------ |
| **🖥️ Kotlin Multiplatformアプリ** | gRPC Stream    | • Kotlin/NativegRPCクライアント<br/>• 最高性能（<30ms）<br/>• Protocol Buffersによる型安全性<br/>• バイナリデータ効率転送             |
...
| 検査マスタ       | `/inspection-targets`         | GET       | 検査対象一覧         | ✅        |
| 検査マスタ       | `/inspection-targets`         | POST      | 検査対象作成         | ✅        |
| 検査マスタ       | `/inspection-targets/{id}`    | GET       | 検査対象詳細         | ✅        |
...
#### 検査対象一覧取得API
```
GET /api/v1/inspection-targets?page=1&per_page=20
...
#### 検査対象詳細取得API
```
GET /api/v1/inspection-targets/{id}
...
| サービス名            | エンドポイント               | タイムアウト | リトライ回数 | 主要機能             |
| --------------------- | ---------------------------- | ------------ | ------------ | -------------------- |
| resize-service        | resize-grpc-app:50051        | 30秒         | 3回          | 画像リサイズ・正規化 |
| ai-detection-service  | ai-detection-grpc-app:50052  | 60秒         | 2回          | AI物体検知・分析     |
| filter-service        | filter-grpc-app:50053        | 20秒         | 3回          | 画像フィルタ・補正   |
| camera-stream-service | camera-stream-grpc-app:50054 | 10秒         | 1回          | リアルタイム映像処理 |
...
## 7.1. Kotlin Multiplatform検査アプリAPI

### 7.1.1. 検査マスタ管理API（製品情報統合）
...
GET /api/v1/inspection/items/by-product-type/{product_type}
...
### 6.3.2. Kotlin Multiplatformアプリ専用エンドポイント
...
# 7. Kotlin MultiplatformからBackend APIへのgRPC通信

## 7.1. Kotlin Multiplatform gRPC クライアント設計
...
### 7.1.3. UI統合設計（Compose Multiplatform）
...
# 10. 録画API設計（ウェアラブル一人称映像）

本章は、THINKLET等のウェアラブルからの録画機能に関するサーバー側の公式契約を定義します。クライアント側の同期アルゴリズムやリングバッファは「0310_KotlinMultiPlatformアプリ設計.md」を参照してください。デバイス固有の運用・QoS方針は「0311_THINKLETアプリ設計.md」を参照。
...
## 10.2. 録画制御 gRPC API（RecordingControlService）
...
## 10.3. WebRTC/WHEP シグナリングとサーバー録画
...
### 11.1.1. VideoAnalysisService - 映像検索
...
message VideoSearchRequest {
  ...
  optional string product_type = 3;       // 型式
