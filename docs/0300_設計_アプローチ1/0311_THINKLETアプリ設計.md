...
graph TB
  ...
  GestureService --> ThinkletHardware
    ...
    NetworkService -.->|Wi-Fi Direct<br/>暗号化ID認証<br/>リアルタイム制御| HandyTerminal
    ...
    HandyTerminal -.->|即座フィードバック<br/>Wi-Fi Direct暗号化| AudioService
...
**音声インターフェース（XFE音声処理・Wi-Fi Direct混線回避）:**
- 主要コマンド: 「検査開始」「検査停止」「次の項目」「結果確認」「ヘルプ」（Wi-Fi Direct暗号化即座実行）
- 製品音声入力: 「型式 TYPE-ABC」「機番 MACH001」（個別SSID認証で端末アプリ連携）
- 状況報告: 「検査完了しました」「不良を検出しました」「接続を確認してください」（混線なし直接フィードバック）
- フィードバック: 認識時の音声応答・端末アプリ画面更新連動（暗号化通信）
- データ記録: 音声コマンド履歴のサーバー同期（後処理）

**ジェスチャーインターフェース（カメラ+IMU統合・Wi-Fi Direct）:**
...
- オフライン対応: ネットワーク障害時のWi-Fi Direct継続・後同期

**フィードバックシステム（端末アプリ協調・混線回避通信優先）:**
- **LEDフィードバック**: デバイス状態表示（Wi-Fi Direct制御）
...
2. **端末アプリ接続**: Wi-Fi Direct暗号化による端末ペアリング（混線回避通信確立）
3. **製品情報受信**: 端末アプリから順序情報を受信（個別SSID暗号化通信）
...
**端末アプリ混線回避連携機能:**
- **自動デバイス検出**: ペアリング済み端末アプリの自動接続（Wi-Fi Direct暗号化）
- **順序情報自動同期**: 端末アプリで選択された製品情報の即座受信（個別SSID） + サーバー記録
- **検査状態同期**: THINKLETの装着状態・検査状況のリアルタイム表示（混線なし通信）
- **操作指示受信**: 端末アプリからの検査開始・停止・切替指示（近接認証実行）
- **結果配信**: 検査結果の即座転送（暗号化）+ データ永続化（サーバー）
- **オフライン継続**: ネットワーク障害時のWi-Fi Direct基本機能維持・後同期

**混線回避通信方式の利点:**

| 通信レイヤー     | Wi-Fi Direct暗号化利点   | サーバー経由の利点         | 統合効果               |
| ---------------- | ------------------------ | -------------------------- | ---------------------- |
| **制御応答性**   | 低遅延・混線なし即座実行 | 安定性・エラー処理         | 高速確実・干渉なし制御 |
| **データ管理**   | 暗号化保存・高速アクセス | 永続化・監査証跡           | セキュア処理・追跡可能 |
| **可用性**       | 混線回避オフライン継続   | 統合管理・Web監視連携      | 干渉なし継続と統合監視 |
| **セキュリティ** | 個別認証・暗号化         | サーバー認証・アクセス制御 | 多層防御・混線防止     |

**混線回避通信品質保証メカニズム:**
- **Wi-Fi Direct監視**: 個別SSID品質・遅延・混線検出監視
- **チャンネル最適化**: 2.4GHz/5GHz動的切替・干渉回避  
...
│                ビジネスロジック層                   │
├─────────────────────────────────────────────────┤
│ 検査制御   │ データ処理 │ 状態管理 │ Wi-Fi管理    │
│ (共通Logic)│ (KMP共通) │(共通状態)│(混線回避)    │
├─────────────────────────────────────────────────┤
│                   通信層                         │
├─────────────────────────────────────────────────┤
│ Wi-Fi Direct │ HTTP通信  │ 暗号化処理│ 認証管理    │
│ (個別SSID)   │(サーバー) │(AES-256) │(干渉防止)   │
├─────────────────────────────────────────────────┤
│                データアクセス層                     │
├─────────────────────────────────────────────────┤
...
| レイヤー               | 技術仕様          | 混線回避仕様                |
| ---------------------- | ----------------- | --------------------------- |
| **直接通信**           | Wi-Fi Direct      | 個別SSID・暗号化・近接認証  |
...
THINKLET-端末ペア     Wi-Fi Direct通信       端末アプリ
┌─────────────┐      (個別SSID暗号化)      ┌─────────────┐
