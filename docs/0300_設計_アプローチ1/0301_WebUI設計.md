...
1. **自然言語指示（オプション）**: 入力欄に処理内容を日本語で記述するとLLMがコンポーネント列とパラメータを提案し、キャンバスに反映。ツールレジストリのメタデータをプロンプトに含め誤使用を防止する。
2. **コンポーネント配置**: パレットからキャンバスへドラッグ&ドロップ
3. **接続定義**: コンポーネント間を接続線で結合
4. **パラメータ設定**: 各コンポーネントの詳細パラメータ設定
5. **実行順序定義**: 直列・並列実行の組み合わせ設定
6. **テスト実行**: サンプル画像での動作確認
7. **保存・公開**: パイプライン定義の保存と検査システムでの利用開始
