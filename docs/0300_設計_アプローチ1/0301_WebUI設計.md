...
- **プライバシー管理**: プライバシーゾーン/ビーコン健全性、入退室イベント監視、原本アクセス承認
...
- **プライバシー指示**: 「プライバシーゾーン中」バッジを常時表示。録画/配信トグルは無効化。
...
| トレーサビリティ管理       | 製品追跡・履歴表示     | ・順序情報取得<br/>・タイムライン表示<br/>・関連ファイル<br/>・詳細情報                          | 時系列表示・ファイル連携           |
...
- 既定はプライバシー処理済み映像を再生。原本は承認後の期限付きトークンでのみ表示

### 6.3.12. プライバシー/ビーコン管理画面（新規）

**一覧機能**:
- ゾーン一覧（`zone_id`、設置場所、ビーコン数、アラート状態）
- ビーコン一覧（`beacon_id`、タイプ、last_seen、vbatt、交換予定日）

**健全性監視**:
- last_seen > 24h で警告、>72h で重大
- 電圧 vbatt < 2.7V で警告
...
**デバイス状態バッジ**:
- ライブ監視や実行ダッシュボードに「プライバシーゾーン中」バッジを表示、録画/配信トグルを無効化
...
- `group_id`: 型式グループコード（グループ指定時に使用）
- `group_name`: グループ名（表示名、任意）

## 7.3. 端末アプリへの反映

- 端末（KMP）アプリは製品選択後にサーバーから検査項目を取得し、検査詳細画面に表示する。
- Webでの変更は即座にサーバー経由で配信され、端末側の検査順序・必須設定に反映される。

## 7.4. 型式グループ管理UI（新規）

- 機能: 型式グループの作成/編集/削除、メンバー（型式コード）の追加/削除。
- 画面構成: グループ一覧（ページング）、詳細編集ダイアログ（名称/説明、メンバー一覧と追加欄）。
- 検査ターゲット作成時: グループ選択（group_id指定）か、製品コード（product_code）単体指定のどちらかを選択可能。
...
- 作成/編集ダイアログ
  - 入力: group 選択（必須）、process 選択（必須）、名称、バージョン、説明、付随情報
  - 備考: 旧 `product_code` 入力欄は廃止（過去データ閲覧時は表示のみ）

### 7.5.2. 型式グループ管理
- 追加: `group_code` 入力（ユニーク）
- メンバー編集: product_code の追加/削除（従来通り）

### 7.5.3. 工程マスタ管理（新規セクション）
- 一覧/検索: process_code, process_name
- 作成/編集/削除

### 7.5.4. 適用の見え方
- 「検査項目」一覧は選択中の（型式グループ, 工程）に紐づくターゲットから取得
- 製品個票からの遷移時は、製品が属する group と選択/推定された process に基づき対象マスタを決定
