...
CREATE TABLE ingest_sessions (
    ...
    state VARCHAR(16) NOT NULL DEFAULT 'active', -- active|stopped|failed|suspended_privacy
...
プライバシーゾーン管理（ビーコン・イベント）
```sql
-- BLEビーコン登録
CREATE TABLE beacons (
    id UUID PRIMARY KEY,
    zone_id VARCHAR(64) NOT NULL,
    beacon_type VARCHAR(16) NOT NULL,     -- eddystone_uid|eddystone_eid|ibeacon
    namespace_id VARCHAR(20),             -- Eddystone UID
    instance_id VARCHAR(20),              -- Eddystone UID
    uuid VARCHAR(36),                     -- iBeacon UUID
    major INTEGER,                        -- iBeacon Major
    minor INTEGER,                        -- iBeacon Minor
    install_location TEXT,
    installed_at TIMESTAMP DEFAULT NOW(),
    expected_replace_by DATE,
    tags JSONB
);
CREATE UNIQUE INDEX idx_beacons_keys ON beacons(zone_id, beacon_type, COALESCE(uuid,'') , COALESCE(namespace_id,''), COALESCE(instance_id,''), COALESCE(major,0), COALESCE(minor,0));

-- 最終検知・電圧監視（TLM）
CREATE TABLE beacon_last_seen (
    beacon_id UUID REFERENCES beacons(id) PRIMARY KEY,
    last_seen_at TIMESTAMP,
    last_rssi INTEGER,
    vbatt_mv INTEGER,
...
-- プライバシー入退室・警告イベント（監査）
CREATE TABLE privacy_zone_events (
    id UUID PRIMARY KEY,
    device_id UUID REFERENCES devices(id),
    session_id UUID REFERENCES ingest_sessions(id),
    zone_id VARCHAR(64),
    event_type VARCHAR(32) NOT NULL,      -- privacy_enter|privacy_exit|privacy_suspend|privacy_resume|beacon_lost|beacon_battery_low
...
CREATE INDEX idx_privacy_events_device_time ON privacy_zone_events(device_id, created_at DESC);
```
---
...
CREATE TABLE thinklet_inspection_segments (
    ...
    duration_seconds     INTEGER,
    object_key           TEXT NOT NULL,      -- MinIOのオブジェクトキー（vms/uplink/...）
...
CREATE TABLE uplink_stream_metrics (
    id BIGSERIAL PRIMARY KEY,
...
- ストリーム品質監視: `uplink_stream_metrics`による接続状態・QoS追跡
...
CREATE INDEX idx_tsm_session_time ON uplink_stream_metrics(ingest_session_id, measured_at DESC);
