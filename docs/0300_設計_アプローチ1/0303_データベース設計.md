...
InspectionTarget との関連（グループ優先の適用）

```sql
-- 検査対象（InspectionTarget）側は、従来の product_code（型式単体）に加えて
...
CREATE INDEX IF NOT EXISTS idx_inspection_targets_group_id
    ON inspection_targets(group_id);
```

適用順序（製品→検査項目）
- 1) 製品の `product_code` が所属する `product_code_groups` を探索し、`inspection_targets.group_id` 一致のターゲットを優先
- 2) 一致が無い場合は `inspection_targets.product_code` ≡ 製品 `product_code`
- 3) 更なるフォールバックとして `inspection_targets.product_code` ≡ 製品 `work_order_id`

### 1.4.1. Kotlin Multiplatformアプリ（SQLite）スキーマ
...
### 1.6.3. Kotlin Multiplatform同期専用アクセスパターン
...
3. `inspection_targets(group_id, process_code)` をキーに対象マスタを取得

### 1.8.3. マイグレーション指針
- 既存 `inspection_targets.product_code` が設定済みの行は、対応するグループへ移管する（移行期間中は両フィールドを許容し、読み取りは group を優先）
