...
注: 現場ネットワーク（工場内Wi‑Fi/AP設計、ローミング、干渉対策、バックホール）の詳細は、`0307_インフラ設計.md` の「8.1.3. 現場ネットワーク設計（小型AP＋有線バックホール）」に集約しています。本書ではサーバー側のプロトコルと連携観点に限定します。
...
Backend APIの実装は、FastAPIとgrpcioを統合したハイブリッドアプローチを採用します。詳細なAPI仕様については、[0304_API設計.md](./0304_API設計.md)を参照してください。
