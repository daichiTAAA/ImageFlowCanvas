...
### 1.2.3. LLM連携パイプライン生成
- LLMアダプタが日本語指示を解析し、ステップとパラメータを抽出。
- ツールレジストリから利用可能なgRPCサービス情報を取得し、生成結果を検証。
- パイプライン生成サービスがワークフロー定義を作成し永続化。
- MLRunオーケストレータがワークフローを実行し、推論サーバへ要求を転送。
- 実行結果は既存のPipeline Executor経由でクライアントへ返却。
