...
flowchart TD
    ...
    DesktopApp -->|gRPC Stream| Envoy  
    MobileApp -->|gRPC Stream| Envoy  
    WebApp -->|gRPC-Web| Envoy  
...
* **データ正規化**: 取得した映像フレームを、システム共通のgRPCメッセージフォーマット（VideoFrame）に変換する。  
* **データ送信**: 正規化されたフレームを、gRPCストリームとしてバックエンドに送信する。
...
* **技術スタック**: React + TypeScript + gRPC-Web  
* **対応ソース**:  
  * PC内蔵カメラ、USB接続カメラ (getUserMedia)  
  * ネットワークカメラ (WHEP)  
* **制約と対応**:  
  * **プロトコル制約**: ブラウザのセキュリティ制約により、RTSPは直接サポートできない。  
  * **通信制約**: gRPCの代わりにgRPC-Web（HTTP/1.1またはHTTP/2上）を使用してバックエンドと通信。  
...
* **Envoy Proxy**:  
  * **役割**: バックエンド全体の統一されたエントリーポイント。  
  * **機能**: gRPCストリームの終端、gRPC-Webサポート、後段のgRPC Gatewayへのルーティング、TLS終端、負荷分散、CORSポリシー適用などを担当する。
...
3. **アプリ**: gRPCストリーム（ネイティブアプリ）またはgRPC-Web（ブラウザ）でEnvoyへ送信。  
4. **バックエンド**: Envoy -> gRPC Gateway -> 各種処理サービスへと流れる。

### **5.2. ネットワークカメラ (RTSP)**

1. **アプリ**: ユーザーが登録したIPアドレスのRTSPストリームに接続。  
2. **アプリ**: 受信したストリームをデコードし、フレームをVideoFrameに変換。  
3. **アプリ**: gRPCストリーム（デスクトップ/モバイル）でEnvoyへ送信。  
4. **バックエンド**: 以降はローカルカメラと同様。  
...
4. **アプリ**: gRPCストリーム（ネイティブアプリ）またはgRPC-Web（ブラウザ）でEnvoyへ送信。  
5. **バックエンド**: 以降はローカルカメラと同様。

## **6. 通信プロトコルとデータフォーマット**

システムのコアとなる通信は**gRPCストリーミング**に統一する。データフォーマットは**Protocol Buffers**で定義し、システム全体での一貫性を保つ。
```protobuf
// camera_stream.proto
