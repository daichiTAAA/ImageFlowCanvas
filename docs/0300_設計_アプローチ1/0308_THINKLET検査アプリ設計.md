...
## 3.3.3. サーバ連携
- `POST /v1/uplink/control` で `privacy_suspend` / `privacy_resume` を送信。`zone_id`,`rssi`,`session_id` を含む。
- `POST /v1/uplink/sessions/{id}/telemetry` に入退室イベントとメトリクスを送信（監査）。
...
検索・再生API仕様は「0304_API設計.md」の VMS セクション（/v1/vms/*）に集約。プレイヤーUIはWeb/端末アプリ設計（0301/0309）で定義。THINKLET側はメタデータ付与とセグメント区切りイベント送出に注力します。
