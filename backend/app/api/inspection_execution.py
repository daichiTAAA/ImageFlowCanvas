...
from app.models.inspection import (
    InspectionTarget, InspectionItem, InspectionExecution, 
    InspectionItemExecution, InspectionResult
)
...
@router.post("/executions", response_model=ExecuteInspectionResponse, tags=["inspection-execution"])
async def create_inspection_execution(
    ...
    try:
        ...
        if not target.inspection_items:
            raise HTTPException(status_code=400, detail="No inspection items defined for this target")
        ...
        logger.info(f"Created inspection execution: {execution.id} for target: {request.target_id}")
        
        return ExecuteInspectionResponse(
            execution_id=execution.id,
            execution=execution
        )
...
@router.get("/executions/{execution_id}", response_model=InspectionExecutionResponse, tags=["inspection-execution"])
    ...
    try:
        execution = await db.execute(
            ...
            .options(
                ...
                selectinload(InspectionExecution.item_executions).selectinload(InspectionItemExecution.item)
            )
        ...
        if not execution:
            raise HTTPException(status_code=404, detail="Inspection execution not found")
...
@router.get("/executions", response_model=PaginatedResponse[InspectionExecutionResponse], tags=["inspection-execution"])
async def list_inspection_executions(
    ...
    try:
        # ベースクエリ
        query = select(InspectionExecution).options(selectinload(InspectionExecution.target))
        ...
        query = query.offset(offset).limit(page_size).order_by(InspectionExecution.started_at.desc())
...
@router.post("/executions/{execution_id}/items/{item_id}/execute", response_model=InspectionItemExecutionResponse, tags=["inspection-execution"])
async def execute_inspection_item(
    ...
    try:
        ...
        if not item_execution:
            raise HTTPException(status_code=404, detail="Inspection item execution not found")
        
        if item_execution.status not in [InspectionItemStatus.ITEM_PENDING, InspectionItemStatus.ITEM_FAILED]:
            raise HTTPException(status_code=400, detail="Item execution not in executable state")
        ...
        try:
            ...
            logger.info(f"Completed AI inspection for item execution: {item_execution.id}")
            ...
            logger.error(f"AI inspection failed for item execution {item_execution.id}: {ai_error}")
            raise HTTPException(status_code=500, detail=f"AI inspection failed: {ai_error}")
...
@router.post("/results", response_model=InspectionResultResponse, tags=["inspection-execution"])
async def save_inspection_result(
    ...
    try:
        # 検査項目実行存在チェック
        item_execution = await db.execute(
            select(InspectionItemExecution).where(InspectionItemExecution.id == request.item_execution_id)
        )
        item_execution = item_execution.scalar_one_or_none()
        ...
        existing_result = await db.execute(
            select(InspectionResult).where(InspectionResult.item_execution_id == request.item_execution_id)
        )
...
@router.get("/results", response_model=PaginatedResponse[InspectionResultResponse], tags=["inspection-execution"])
async def list_inspection_results(
    ...
    try:
        ...
        if target_id:
            query = query.join(InspectionExecution).where(InspectionExecution.target_id == target_id)
            count_query = count_query.join(InspectionExecution).where(InspectionExecution.target_id == target_id)
        ...
        # ページング
        offset = (page - 1) * page_size
        query = query.offset(offset).limit(page_size).order_by(InspectionResult.created_at.desc())
...
@router.get("/executions/{execution_id}/items", response_model=List[InspectionItemExecutionResponse], tags=["inspection-execution"])
async def get_inspection_item_executions(
    ...
    try:
        item_executions = await db.execute(
            ...
            .order_by(InspectionItemExecution.item.has(execution_order=None))  # execution_orderでソート
        )
...
@router.put("/executions/{execution_id}/status", response_model=InspectionExecutionResponse, tags=["inspection-execution"])
async def update_inspection_execution_status(
    ...
    try:
        ...
        if status in [InspectionStatus.COMPLETED, InspectionStatus.FAILED, InspectionStatus.CANCELLED]:
