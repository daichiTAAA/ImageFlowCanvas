...
class InspectionExecutor:
        ...
        try:
            ...
            if not item_execution.item:
                item = await self.db.execute(
                    select(InspectionItem).where(InspectionItem.id == item_execution.item_id)
                )
                item = item.scalar_one_or_none()
                if not item:
                    raise ValueError(f"Inspection item not found: {item_execution.item_id}")
            ...
            if not item_execution.item.pipeline_id:
                logger.warning(f"No pipeline defined for inspection item: {item_execution.item_id}")
                return None
            ...
            if not pipeline_result:
                logger.warning(f"Pipeline execution returned no result for item: {item_execution.item_id}")
                return None
    ...
    def _parse_pipeline_result(
        ...
        try:
            # パイプライン結果の構造を分析
            if hasattr(pipeline_result, 'result') and hasattr(pipeline_result.result, 'output_data'):
                # 標準的なパイプライン結果形式
                output_data = pipeline_result.result.output_data
                processing_time = getattr(pipeline_result.result, 'processing_time_seconds', 0) * 1000
            else:
                # 直接的な結果形式
        ...
        try:
            ...
            if criteria_spec.get('binary'):
                ...
                if operator == 'GREATER_THAN':
                    ...
                    return JudgmentResult.OK if abs(value - threshold) < 0.01 else JudgmentResult.NG
                else:
                    return JudgmentResult.OK if abs(value - threshold) >= 0.01 else JudgmentResult.NG
