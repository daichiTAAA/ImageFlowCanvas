...
class ArgoWorkflowService:
    ...
    def __init__(self):
        self.argo_server_url = os.getenv("ARGO_SERVER_URL", "http://argo-server.argo.svc.cluster.local:2746")
    ...
    def _build_workflow_payload(
        ...
        workflow_payload = {
            ...
            "spec": {
                "workflowTemplateRef": {
                    "name": self.workflow_template
                },
                "arguments": {
                    "parameters": [
                        {
                            "name": "input-files",
                            "value": json.dumps(input_files)
                        },
                        {
    ...
    def _build_dynamic_workflow(
        ...
        for i, component in enumerate(components):
            ...
            template_name = template_mapping.get(component_type, "generic-container-template")
            ...
            if dependencies:
                ...
                task["dependencies"] = [f"{prev_component.get('name', f'step-{i-1}')}-task"]
    ...
    def _build_component_parameters(
        ...
        base_params = [
            {
                "name": "input-path",
                "value": f"input/step-{step_index}" if step_index == 0 else f"processed/step-{step_index-1}"
            },
            {
                "name": "output-path", 
        ...
        if component_type == "resize":
            base_params.extend([
                ...
                {"name": "height", "value": str(component_params.get("height", 600))},
                {"name": "maintain-aspect", "value": str(component_params.get("maintain_aspect", True)).lower()}
            ])
            ...
            base_params.extend([
                {"name": "filter-type", "value": component_params.get("filter_type", "blur")},
                {"name": "intensity", "value": str(component_params.get("intensity", 1.0))}
            ])
