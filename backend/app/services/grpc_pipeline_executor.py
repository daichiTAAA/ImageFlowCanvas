...
try:
    ...
    logger.warning(f"Could not import generated protobuf files: {e}")
    # Create mock classes for development
    class MockPb2:
        pass
    class MockGrpc:
        class ResizeServiceStub:
            def __init__(self, channel): pass
            async def ResizeImage(self, request, timeout=None): 
                raise RuntimeError("gRPC services not available")
        class AIDetectionServiceStub:
            def __init__(self, channel): pass
            async def DetectObjects(self, request, timeout=None):
                raise RuntimeError("gRPC services not available")
        class FilterServiceStub:
            def __init__(self, channel): pass
            async def ApplyFilter(self, request, timeout=None):
                raise RuntimeError("gRPC services not available")
    
    resize_pb2 = MockPb2()
    resize_pb2_grpc = MockGrpc()
    ai_detection_pb2 = MockPb2()
    ai_detection_pb2_grpc = MockGrpc()
    filter_pb2 = MockPb2()
    filter_pb2_grpc = MockGrpc()
    common_pb2 = MockPb2()
...
class GRPCPipelineExecutor:
    ...
    def _initialize_grpc_connections(self):
        ...
        for service_name, config in self.grpc_services.items():
            try:
                ...
                logger.info(f"Initialized gRPC connection to {service_name} at {config['endpoint']}")
        ...
        try:
            ...
            for step_group in execution_order:
                if len(step_group) == 1:
                    ...
                    await self._send_progress_notification(execution_id, step['stepId'], 'completed', result)
                    ...
                    for step, task in tasks:
                        try:
                            ...
                            logger.error(f"Failed to execute step {step['stepId']}: {e}")
                            results[step['stepId']] = {
                                'status': 'failed',
                                'error': str(e)
                            }
                            await self._send_progress_notification(execution_id, step['stepId'], 'failed', {'error': str(e)})
    ...
    async def _execute_step(self, step: Dict[str, Any], input_path: str, execution_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"Executing step {step_id} ({component_name}) for execution {execution_id}")
        ...
        try:
            if component_name == 'resize':
                ...
                result = await self._execute_ai_detection_step(step, input_path, execution_id)
    ...
    async def _execute_resize_step(self, step: Dict[str, Any], input_path: str, execution_id: str) -> Dict[str, Any]:
        """Execute resize step via direct gRPC call"""
        parameters = step.get('parameters', {})
        
        # Create gRPC request
        request = resize_pb2.ResizeImageRequest(
            input_image=common_pb2.ImageReference(
        ...
        return {
            ...
            'metadata': {
                'original_width': response.original_width,
                'original_height': response.original_height,
                'new_width': response.new_width,
                'new_height': response.new_height,
                'processing_time_ms': response.processing_time_ms
            }
        }
    
    async def _execute_ai_detection_step(self, step: Dict[str, Any], input_path: str, execution_id: str) -> Dict[str, Any]:
        """Execute AI detection step via direct gRPC call"""
        parameters = step.get('parameters', {})
        
        # Create gRPC request
        request = ai_detection_pb2.DetectObjectsRequest(
            input_image=common_pb2.ImageReference(
                bucket='imageflow-files',
                object_key=input_path,
                content_type='image/jpeg'
    ...
    async def _execute_filter_step(self, step: Dict[str, Any], input_path: str, execution_id: str) -> Dict[str, Any]:
        """Execute filter step via direct gRPC call"""
        parameters = step.get('parameters', {})
        
        # Create gRPC request
        request = filter_pb2.ApplyFilterRequest(
            input_image=common_pb2.ImageReference(
                bucket='imageflow-files',
                object_key=input_path,
                content_type='image/jpeg'
            ),
            filter_type=self._get_filter_type_enum(parameters.get('filter_type', 'gaussian')),
    ...
    def _get_filter_type_enum(self, filter_type_str: str):
        ...
        filter_map = {
            ...
            'edge': filter_pb2.FilterType.FILTER_TYPE_EDGE,
            'emboss': filter_pb2.FilterType.FILTER_TYPE_EMBOSS
        }
        return filter_map.get(filter_type_str.lower(), filter_pb2.FilterType.FILTER_TYPE_GAUSSIAN)
    
    def _resolve_step_dependencies(self, steps: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        """
        ...
        while len(executed) < len(steps):
            ...
            if not ready_steps:
                ...
                remaining_steps = [step_map[step_id] for step_id in dependencies.keys() if step_id not in executed]
    ...
    async def _send_progress_notification(self, execution_id: str, step_id: str, status: str, data: Dict[str, Any]):
