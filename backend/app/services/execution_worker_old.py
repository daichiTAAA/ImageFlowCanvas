...
class ExecutionWorker:
            ...
            try:
                ...
                for execution in pending_executions:
                    ...
                    if hasattr(execution, 'created_at'):
                        time_diff = (datetime.now() - execution.created_at).total_seconds()
                        if time_diff > 30:
                            logger.warning(f"Retrying stale execution {execution.execution_id}")
    ...
    async def retry_stale_execution(self, execution):
        ...
        try:
            ...
            result = await self.grpc_executor.execute_pipeline(pipeline_config, execution.execution_id)
            ...
            logger.error(f"Error retrying stale execution {execution.execution_id}: {e}")
        ...
        try:
            ...
            for status in health_statuses:
                if status.get('status') != 'healthy':
                    logger.warning(f"gRPC service {status.get('service_name')} is unhealthy: {status.get('error')}")
